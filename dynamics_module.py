import numpy as np
# Lazy matplotlib import: allows non-plotting usage without matplotlib deps
try:
...
def projector_centered_sp(L, size, bc='open', dtype=np.float64):
    """
    P on region A = [center-L, center+L) centered at 'size' in a chain of length N=2*size.
    This yields 2*L sites (half-open interval).
    ...
    if bc == 'open':
        ...
        idx = (np.arange(center - L, center + L) % N)
        diag[idx] = 1.0
    else:
...
def projector_RIGHT_sp(LA, L, N, bc='open', dtype=np.float64):
    """
    P on region A = {LA,..., LA+L}. (Useful for a half-chain etc.)
    Returns sparse diagonal (CSR). For PBC, same diagonal (wrap not needed).
    """
    diag = np.zeros(N, dtype=dtype)
    length = LA + L + 1
