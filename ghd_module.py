import numpy as np
# Lazy matplotlib import so non-plotting code can run even if deps are missing
try:
...
def export_symm_minus_csv(Q_list, J_list, times, gamma, out_csv, bc="obc", r=1, two_site_avg=True):
    ...
    """
    # Allow scalar/array inputs; normalize to list-of-arrays and matching times
    if np.isscalar(times) or not hasattr(times, "__len__"):
        times = [times]
    else:
        times = list(times)
    ...
    if isinstance(Q_list, np.ndarray):
        if Q_list.ndim == 1:
            ...
            Q_list = [Q_list[i, :] for i in range(Q_list.shape[0])]
        else:
            Q_list = list(Q_list)
    elif not hasattr(Q_list, "__len__"):
        Q_list = [Q_list]

    if isinstance(J_list, np.ndarray):
        if J_list.ndim == 1:
            J_list = [J_list]
        elif J_list.ndim == 2:
            J_list = [J_list[i, :] for i in range(J_list.shape[0])]
        else:
            J_list = list(J_list)
    elif not hasattr(J_list, "__len__"):
        J_list = [J_list]

    # Broadcast times if a single time is given for multiple profiles
    if len(times) == 1 and len(Q_list) > 1:
        times = times * len(Q_list)
    if len(Q_list) == 1 and len(times) > 1:
        Q_list = Q_list * len(times)
    if len(J_list) == 1 and len(times) > 1:
        J_list = J_list * len(times)
...
def chi_plus_MICHELE(k, gamma):
    """
    J=1
    \chi⁺(k) = \theta(-k) J/4(\pi J + \gamma) +\theta(k) \gamma / 4\pi(\pi J + \gamma) 

    k : float
    r_vals, b_vals : arrays of same shape (e.g. output of solve_truncated_system_full)
    """
    if k > 0:
        return gamma/(4*np.pi*(np.pi + gamma)) 
    if k < 0:
        return 1/(4*(np.pi + gamma)) 
    # k == 0: average the two branches
    return 0.5*(gamma/(4*np.pi*(np.pi + gamma)) + 1/(4*(np.pi + gamma)))
...
def n_zeta_MICHELE(k, zeta, gamma):
    
    """
    n_\zeta(k) for Micheles solution
    We use ε'(k) = 2 sin(k) (J=1 convention).
    """
    epsp = 2.0 * np.sin(k)
    chi  = chi_plus_MICHELE(k, gamma)
...
def n_thermal(Beta,k):
    eps = -2.0 * np.cos(k)
    return 1/(np.exp(Beta*eps)+1)


def n_zeta_thermal(k, BetaL, BetaR, zeta):
    epsp = 2.0 * np.sin(k)
    if BetaL == 0:
        return theta(epsp- zeta)*(n_L(k)/(2*np.pi))+ theta(-epsp + zeta)*(n_thermal(BetaR, k)/(2*np.pi))
    else:
        return theta(epsp- zeta)*(n_thermal(BetaL, k)/(2*np.pi))+ theta(-epsp + zeta)*(n_thermal(BetaR, k)/(2*np.pi))
    
def n_zeta_Q2(k, BetaR, zeta, J=1.0, BetaL=0.0):
    v = 2.0*J*np.sin(k)              # group velocity of H
    q2 = 2.0*J*np.cos(2*k)           # Q2 eigenvalue
    n_Q2_R = 1.0/(np.exp(BetaR*q2)+1.0)
    if BetaL == 0.0:
        n_Q2_L = n_L(k) / (2*np.pi)
    else:
        n_Q2_L = (1.0/(np.exp(BetaL*q2)+1.0)) / (2*np.pi)
    n_Q2_R = n_Q2_R / (2*np.pi)
    return theta(v - zeta)*n_Q2_L + theta(-v + zeta)*n_Q2_R
...
def hyd_charge_MICHELE(r, zeta, sign, gamma):
    """
    ⟨ q^{(r,sign)} ⟩_{n_\zeta}
      = ∫ dk q^{(r,sign)}(k) n_\zeta(k)
      Using Michele's solution
    """
    qfun = q_minus if sign == '-' else q_plus

    def f(k):
        return qfun(k, r) * n_zeta_MICHELE(k, zeta, gamma)

    val, _ = quad(f, -np.pi, np.pi, limit=500)
    return val

def hyd_charge_thermal(r, zeta, sign, BetaL,  BetaR):
    """
    ⟨ q^{(r,sign)} ⟩_{n_\zeta}
      = ∫ dk q^{(r,sign)}(k) n_\zeta(k)
    """
    ...
    def f(k):
        return qfun(k, r) * n_zeta_thermal(k, BetaL, BetaR, zeta)

    val, _ = quad(f, -np.pi, np.pi, limit=500)
    return val


def hyd_charge_Q2(r, zeta, sign, BetaR, BetaL=0.0):
    """
    ⟨ q^{(r,sign)} ⟩_{n_\zeta}
      = ∫ dk q^{(r,sign)}(k) n_\zeta(k)
    """
    ...
    def f(k):
        return qfun(k, r) * n_zeta_Q2(k, BetaR, zeta, BetaL=BetaL)
...
def hyd_current_MICHELE(r, zeta, sign, gamma):
    """
    ⟨ J^{(r,sign)} ⟩_{n_\zeta}
      = ∫ dk ε'(k) q^{(r,sign)}(k) n_\zeta(k)
    with ε'(k) = 2 sin(k).
    ...
    def f(k):
        return 2.0 * np.sin(k) * qfun(k, r) * n_zeta_MICHELE(k, zeta, gamma)

    val, _ = quad(f, -np.pi, np.pi, limit=400)
    return val

def hyd_current_thermal(r, zeta, sign, BetaL,  BetaR):
    """
    ⟨ J^{(r,sign)} ⟩_{n_\zeta}
      = ∫ dk ε'(k) q^{(r,sign)}(k) n_\zeta(k)
    with ε'(k) = 2 sin(k).
    ...
    def f(k):
        return 2.0 * np.sin(k) * qfun(k, r) * n_zeta_thermal(k, BetaL, BetaR, zeta)

    val, _ = quad(f, -np.pi, np.pi, limit=400)
    return val


def hyd_current_Q2(r, zeta, sign, BetaR, BetaL=0.0):
    """
    ⟨ J^{(r,sign)} ⟩_{n_\zeta}
      = ∫ dk ε'(k) q^{(r,sign)}(k) n_\zeta(k)
    with ε'(k) = 2 sin(k).
    ...
    def f(k):
        return 2.0 * np.sin(k) * qfun(k, r) * n_zeta_Q2(k, BetaR, zeta, BetaL=BetaL)
...
def compare_lattice_vs_ghd(C_zeta, N, t, r,
    ...
    # \zeta-axis: align with two-site averaging (centered between sites)
    xs = np.arange(0, N - 1)
    center = (N - 1) / 2.0
    XIS = (xs - center + 0.5) / t

    # LATTICE with two-site averaging inline to match CSV output
    if sign == '-':
        q_site = np.array([qm_symm(int((r-1)/2), xx, C_zeta, bc).real for xx in range(N)])
        J_site = np.array([jm_symm(int((r-1)/2), xx, C_zeta, bc).real for xx in range(N)])
    else:
        q_site = np.array([qp_symm(2*r, xx, C_zeta, bc).real for xx in range(N)])
        J_site = np.array([jp_symm(2*r, xx, C_zeta, bc).real for xx in range(N)])
    ...
    #GHD Michele
    q_hydro_MICHELE = np.array([hyd_charge_MICHELE(r, z, sign, gamma) for z in XIS])
    J_hydro_MICHELE = np.array([hyd_current_MICHELE(r, z, sign, gamma) for z in XIS])

    # Plot
    plt.figure(figsize=(10, 6))
    plt.scatter(XIS, q_lat, marker='o', s=5, c='blue', label=f"Numerics q, r={r}")
    plt.scatter(XIS, J_lat, marker='o', s=5, c='red', label=f"Numerics J, r={r}")
    # plt.plot(XIS, q_hydro, '-', lw=2, label=f"GHD q, r={r}")
    # plt.plot(XIS, J_hydro, '-', lw=2, label=f"GHD J, r={r}")
    plt.plot(XIS, q_hydro_MICHELE, '-', lw=2, c= 'green', label=f"GHD q, r={r}")
    plt.plot(XIS, J_hydro_MICHELE, '-', lw=2, c='orange', label=f"GHD J, r={r}")
    plt.xlabel(r'$\zeta$')
