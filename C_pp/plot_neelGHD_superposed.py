...
def main():
    ...
    ap = argparse.ArgumentParser(description="Overlay q/J profiles grouped by r, gamma and/or time.")
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "'profile_grid' builds a multi-panel overview with (q,J) pairs and up to "
            "two r values per row."
        ),
    )
    ...
    if args.group_mode == "profile_grid":
        ...
        rows = 2
        cols = n_profiles
        fig_width = max(6.4, 2.9 * cols)
        fig, axes = plt.subplots(rows, cols, figsize=(fig_width, 5.5), sharex="col", squeeze=False)
        used_metas = []
        for col, r_val in enumerate(r_order):
            ...
            for idx_meta, meta in enumerate(col_metas):
                ...
                draw_group(meta, (q_ax, j_ax), label_prefix=label_prefix, color_offset=3 * idx_meta, legend_mode="none")
        ...
        for col in range(cols):
            ...
            axes[1, col].set_xlabel(r"$\zeta$", labelpad=8)
            axes[0, col].yaxis.tick_left()
            axes[1, col].yaxis.tick_left()
            axes[1, col].yaxis.set_label_position("left")
            if col == 0:
                axes[0, col].set_ylabel(r"$q(\zeta)$", labelpad=25)
                axes[1, col].set_ylabel(r"$J(\zeta)$", labelpad=25)
                axes[0, col].yaxis.set_label_coords(-0.23, 0.5)
                axes[1, col].yaxis.set_label_coords(-0.23, 0.5)
            else:
        ...
        for meta in group_metas:
            fig, axes = plt.subplots(1, 2, figsize=(13.0, 5.8), sharex=True)
            draw_group(meta, axes, legend_mode="both")
            axes[0].set_xlabel(r"$\zeta$", labelpad=8)
            axes[1].set_xlabel(r"$\zeta$", labelpad=8)
            if args.show_title:
                fig.suptitle(meta["title"], fontsize=16, y=0.98)
                fig.subplots_adjust(left=0.08, right=0.98, bottom=0.16, top=0.86, wspace=0.28)
            else:
                fig.subplots_adjust(left=0.08, right=0.98, bottom=0.16, top=0.96, wspace=0.28)
