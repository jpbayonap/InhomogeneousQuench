...
def main():
    ...
    ap.add_argument("--time", type=float, default=200, help="Time T (single value)")
    ...
    ap.add_argument("--b-offset", type=int, nargs="+", default=[1], help="Right-region length list (B = [center+1, center+1+b_offset))")
    ...
    tasks = [(s, b_off, g) for s in args.sizes for b_off in b_offset for g in args.gammas]

    def run_task(s, b_off, g):
        N = 2 * s
        center = N // 2
        a_start= max(0, center-a_offset)
        a = np.arange(a_start, center+1, dtype=int)
        b_end = min(N, center + 1 + b_off)
        ...
        if local_betaL is not None or local_betaR is not None:
            print(
                f"Time evolution starting: betaL={local_betaL}, betaR={local_betaR}, N={N}, T={T}, gamma={g}, r={r}, sign={sign}, a={a_offset}, b={b_off}"
            )
        elif Beta ==0:
            print(
                f"Time evolution starting: N={N}, T={T}, gamma={g}, r={r}, sign={sign}, a={a_offset}, b={b_off}"
            )
        else:
            print(
                f"Time evolution starting: Beta={Beta}, N={N}, T={T}, gamma={g}, r={r}, sign={sign}, a={a_offset}, b={b_off}"
            )
        ...
        if local_betaL is not None or local_betaR is not None:
            outcsv = os.path.join(
                csv_dir,
                f"GHD_NEEL_betaL_{local_betaL}_betaR_{local_betaR}_r{r}_a_{a_offset}_b_{b_off}_sign{sign}_gamma{g:.2f}_T{T}_N{N}.csv",
            )
        elif Beta==0:
            outcsv = os.path.join(
                csv_dir,
                f"GHD_NEEL_r{r}_a_{a_offset}_b_{b_off}_sign{sign}_gamma{g:.2f}_T{T}_N{N}.csv",
            )
        else:
            outcsv = os.path.join(
                csv_dir,
                f"GHD_NEEL_Beta_{Beta}_r{r}_a_{a_offset}_b_{b_off}_sign{sign}_gamma{g:.2f}_T{T}_N{N}.csv",
            )
        ...
        axes[1].set_ylabel("J")
        if local_betaL is not None or local_betaR is not None:
            fig.suptitle(rf"$\beta_L={local_betaL},\ \beta_R={local_betaR},\ a={a_offset},\ b={b_off},\ \gamma={g},\ T={T},\ N={N},\ r={r},\ \text{{sign}}={sign}$")
        elif Beta==0:
            fig.suptitle(rf"$a={a_offset},\ b={b_off},\ \gamma={g},\ T={T},\ N={N},\ r={r},\ \text{{sign}}={sign}$")
        else:
            fig.suptitle(rf"$\beta={Beta},\ a={a_offset},\ b={b_off},\ \gamma={g},\ T={T},\ N={N},\ r={r},\ \text{{sign}}={sign}$")
    ...
    Parallel(n_jobs=n_jobs)(
        delayed(run_task)(s, b_off, g) for (s, b_off, g) in tasks
    )
