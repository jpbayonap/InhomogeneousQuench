...
def main():
    ...
    def run_task(s, T, g, L):
        ...
        P_cent= projector_centered_sp(L= Length, size= s, bc='open')
        ...
        def matvec(v):
            ...
            term3 = 2.0 * g * (P_cent @ C @ P_cent.conjugate().T)
            return (term1 + term2 + term3).reshape(-1)

        t0 = time.perf_counter()
        print(f"Time evolution starting: N={N}, T={T}, gamma={g}, r={args.r}, sign={sign}, length={2*Length} sites")
        if method == "rk4":
            def rk_step(v, dt):
                k1 = matvec(v)
        ...
        csv_dir = os.path.join(args.outdir, "GHD_TH_PTSYMM_CSV")
        png_dir = os.path.join(args.outdir, "GHD_TH_PTSYMM_PNG")
        ...
        outcsv = os.path.join(
            csv_dir,
            f"GHD_TH_beta_{betaL}_l{L}_r{args.r}_sign{sign}_gamma{g:.2f}_T{T}_N{N}.csv",
        )
