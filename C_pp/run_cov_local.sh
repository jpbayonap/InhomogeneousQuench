...
echo "VAC_FILL=${VAC_FILL} | VAC_INFTY=${VAC_INFTY} | MIXED_NEEL=${MIXED_NEEL} | PHSYMM=${PHSYMM} | PHSYMM_ODD=${PHSYMM_ODD} (m=${PHSYMM_M}, A=${PHSYMM_A})"
...
if [ -z "${BETA_L}" ] && [ -z "${BETA_R}" ]; then CMD+=(--beta "${BETA}"); fi
if [ "${VAC_FILL}" != "0" ]; then CMD+=(--vac-fill); fi
if [ "${VAC_INFTY}" != "0" ]; then CMD+=(--vac-infty); fi
if [ "${MIXED_NEEL}" != "0" ]; then CMD+=(--mixed-neel); fi
if [ "${PHSYMM}" != "0" ]; then CMD+=(--phsymm --phsymm-m "${PHSYMM_M}" --phsymm-A "${PHSYMM_A}"); fi
if [ "${PHSYMM_ODD}" != "0" ]; then CMD+=(--phsymm-odd --phsymm-m "${PHSYMM_M}" --phsymm-A "${PHSYMM_A}"); fi
