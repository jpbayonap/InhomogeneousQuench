...
python3 plot_it_superposed.py \
  ...
  --betaR "0.1"\
  --time 200 \
  --s-offsets "1 5 10 50 100 500 1000"\
  --beta "1.0" \
...
def main():
    ap = argparse.ArgumentParser(description="Overlay q/J profiles for each (r,gamma) with multiple S_OFFSET values.")
    ...
    ap.add_argument("--s-offsets", type=str, default=None, help="Comma/space-separated S_OFFSET values.")
    args = ap.parse_args()
    ...
    for fname in os.listdir(csv_dir):
        ...
        if info is None:
            continue
        if Beta is not None:
            if info["beta"] is None or not np.isclose(info["beta"], Beta):
                continue
        ...
        if n_list is not None and info["N"] not in n_list:
            continue
        if s_offset_list is not None and info["s"] is not None and info["s"] not in s_offset_list:
            continue
    ...
    for (beta_val, r_val, gamma_val), items in groups.items():
        ...
        info = items[0][1]
        if beta_val == 0:
            title = rf"$r={info['r']},\ \mathrm{{sign}}={info['sign']},\ \gamma={info['gamma']},\ T={info['T']},\ N={info['N']}$"
        elif beta_val is None and info.get("betaL") is not None and info.get("betaR") is not None:
            title = rf"$\beta_L={info['betaL']},\ \beta_R={info['betaR']},\ r={info['r']},\ \mathrm{{sign}}={info['sign']},\ \gamma={info['gamma']},\ T={info['T']},\ N={info['N']}$"
        else:
            title = rf"$\beta={beta_val},\ r={info['r']},\ \mathrm{{sign}}={info['sign']},\ \gamma={info['gamma']},\ T={info['T']},\ N={info['N']}$"
        fig.suptitle(title)
        fig.tight_layout()
        if beta_val == 0:
            outpng = os.path.join(png_dir, f"GHD_IT_NEEL_superposed_r{r_val}_gamma{gamma_val:.2f}_T{info['T']}_N{info['N']}.png")
        elif beta_val is None and info.get("betaL") is not None and info.get("betaR") is not None:
            outpng = os.path.join(png_dir, f"GHD_IT_superposed_betaL{info['betaL']}_betaR{info['betaR']}_r{r_val}_gamma{gamma_val:.2f}_T{info['T']}_N{info['N']}.png")            
        else:
            outpng = os.path.join(png_dir, f"GHD_IT_superposed_Beta{beta_val}_r{r_val}_gamma{gamma_val:.2f}_T{info['T']}_N{info['N']}.png")            
