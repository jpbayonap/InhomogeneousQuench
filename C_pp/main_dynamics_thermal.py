...
def main():
    ...
    def run_task(s, T, g):
        ...
        H_L = H_dense[:s, :s]
        H_R = H_dense[s:, s:]
        # Build correlations from Fermi factors; allow empty left if betaL=0 sentinel
        if betaL != betaR:
            C_L = np.zeros((s, s), dtype=complex) if args.betaL == 0.0 else Gamma_beta(betaL, H_L)
        ...
        print(f"Time evolution starting: N={N}, T={T}, gamma={g}, r={args.r}, sign={sign}")
        if method == "rk4":
            def rk_step(v, dt):
                k1 = matvec(v)
        ...
        csv_dir = os.path.join(args.outdir, "GHD_THERM_ALL_CSV")
        png_dir = os.path.join(args.outdir, "GHD_THERM_ALL_PNG")
        ...
        if betaL == 0:
            outcsv = os.path.join(
                csv_dir,
                f"GHD_THERM_LVacuum_betaR_{betaR}_r{args.r}_sign{sign}_gamma{g:.2f}_T{T}_N{N}.csv",
            )
        else:
            outcsv = os.path.join(
                csv_dir,
                f"GHD_THERM_betaL_{betaL}betaR_{betaR}_r{args.r}_sign{sign}_gamma{g:.2f}_T{T}_N{N}.csv",
            )
        ...
        # Symmetric averages for plotting (q even, J odd).
        q_plot = q_vals 
        j_plot = j_vals 

        fig, axes = plt.subplots(1, 2, figsize=(12, 5), sharex=True)
        axes[0].scatter(zetas, q_plot, s=4, c="blue", alpha=0.6, label="q (sym)")
        axes[1].scatter(zetas, j_plot, s=4, c="red", alpha=0.6, label="J (sym)")
